...
export class AmpTrackLoadingService {
    ...
    setProgress(value: number): void {
        if (value < 0 || value > 100) {
            console.error('¡El valor del progreso debe estar entre 0 y 100!');
            return;
        }
    ...
    _setLoadingStatus(status: boolean, url: string): void {
        if (!url) {
            console.error('¡Se debe proporcionar la URL de la solicitud!');
            return;
        }
