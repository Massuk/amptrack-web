...
import {
    AmpTrackNavigationItem,
    AmpTrackVerticalNavigationAppearance,
    AmpTrackVerticalNavigationMode,
    AmpTrackVerticalNavigationPosition,
} from '@amptrack/components/navigation/navigation.types';
import { AmpTrackVerticalNavigationAsideItemComponent } from '@amptrack/components/navigation/vertical/components/aside/aside.component';
...
import { AmpTrackScrollbarDirective } from '@amptrack/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'amptrack-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: amptrackAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'amptrackVerticalNavigation',
    standalone: true,
    imports: [
        AmpTrackScrollbarDirective,
        AmpTrackVerticalNavigationAsideItemComponent,
...
export class AmpTrackVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: AmpTrackVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: AmpTrackVerticalNavigationMode = 'side';
    @Input() name: string = this._amptrackUtilsService.randomId();
    @Input() navigation: AmpTrackNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: AmpTrackVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<AmpTrackVerticalNavigationAppearance> =
        new EventEmitter<AmpTrackVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<AmpTrackVerticalNavigationMode> =
        new EventEmitter<AmpTrackVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<AmpTrackVerticalNavigationPosition> =
        new EventEmitter<AmpTrackVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<AmpTrackNavigationItem> =
        new ReplaySubject<AmpTrackNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<AmpTrackNavigationItem> =
        new ReplaySubject<AmpTrackNavigationItem>(1);
    ...
    private _amptrackScrollbarDirectives!: QueryList<AmpTrackScrollbarDirective>;
    private _amptrackScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'amptrack-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`amptrack-vertical-navigation-appearance-${this.appearance}`]: true,
            'amptrack-vertical-navigation-hover': this._hovered,
            'amptrack-vertical-navigation-inner': this.inner,
            'amptrack-vertical-navigation-mode-over': this.mode === 'over',
            'amptrack-vertical-navigation-mode-side': this.mode === 'side',
            'amptrack-vertical-navigation-opened': this.opened,
            'amptrack-vertical-navigation-position-left': this.position === 'left',
            'amptrack-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for amptrackScrollbarDirectives
     */
    @ViewChildren(AmpTrackScrollbarDirective)
    set amptrackScrollbarDirectives(
        amptrackScrollbarDirectives: QueryList<AmpTrackScrollbarDirective>
    ) {
        // Store the directives
        this._amptrackScrollbarDirectives = amptrackScrollbarDirectives;

        // Return if there are no directives
        if (amptrackScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._amptrackScrollbarDirectivesSubscription) {
            this._amptrackScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._amptrackScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                amptrackScrollbarDirectives.forEach((amptrackScrollbarDirective) => {
                    amptrackScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.amptrack-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._amptrackScrollbarDirectives.forEach(
                    (amptrackScrollbarDirective) => {
                        // Skip if not enabled
                        if (!amptrackScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        amptrackScrollbarDirective.scrollToElement(
                            '.amptrack-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: AmpTrackNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: AmpTrackNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('amptrack-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'amptrack-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'amptrack-vertical-navigation-aside-overlay'
        );
